...
(function () {
  ...
  // show the visitor was told what the data is for, and that is the whole
  // of its job.
  ...
  /* --- Form controls the browser draws for itself ------------------------
     ...
     can reach either, so on this site they arrive grey and square in the
     middle of a dark amber form. The native controls stay in the markup —
     they hold the value and they are what submits — and these draw a
     matching control over the top. With JS off, the natives are what the
     visitor gets, untouched and perfectly usable.                       */
  ...
  function enhanceDate(input) {
    ...
    toggle.setAttribute('aria-label', 'Wybierz termin z kalendarza');
    ...
    // The day the grid is built around: what is chosen, else today.
    ...
    function parsed() {
      var parts = /^(\d{4})-(\d{2})-(\d{2})$/.exec(input.value);
  ...
  if (form) {
    ...
    if (type) enhanceSelect(type);
    if (people) enhanceNumber(people);
    if (date) enhanceDate(date);
